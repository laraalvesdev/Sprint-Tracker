...
export class BoardService {
  ...
  async create(ownerId: string, dto: CreateBoardDto) {
    ...
    await this.prisma.boardMember.create({
      data: { boardId: board.id, userId: ownerId, role: 'ADMIN' },
    });
  ...
  async update(boardId: string, userId: string, dto: UpdateBoardDto) {
    ...
    this.boardGateway.emitModifiedInBoard(boardId, {
      boardId,
      action: 'updated',
  ...
  async remove(boardId: string, userId: string) {
    await assertBoardOwner(this.prisma, boardId, userId);
  ...
  async listMembers(boardId: string, userId: string) {
    await assertBoardObserver(this.prisma, boardId, userId);
  ...
  async removeMember(
    ...
    if (memberUserId === board.ownerId) {
      ...
      const nextAdmin = await this.prisma.boardMember.findFirst({
        where: { boardId, role: 'ADMIN', userId: { not: requesterId } },
        orderBy: { joinedAt: 'asc' },
      });

      if (!nextAdmin) {
        const nextMember = await this.prisma.boardMember.findFirst({
          where: { boardId, role: 'MEMBER', userId: { not: requesterId } },
          orderBy: { joinedAt: 'asc' },
        });
        ...
        await this.prisma.$transaction(async (tx) => {
          await tx.boardMember.update({
            where: { boardId_userId: { boardId, userId: nextMember.userId } },
            data: { role: 'ADMIN' },
          });
    ...
    if (requesterId !== memberUserId) {
      const requesterAccess = await resolveBoardAccess(
        this.prisma,
        boardId,
        requesterId,
      );
  ...
  async responseInvite(
    ...
    await this.prisma.$transaction(async (prisma) => {
      await prisma.boardMember.create({
        data: { boardId, userId: recipientId, role: invite.role },
      });
